...
const DEMO_STORY_CARDS: DerivedArtifact<StoryCardsV1> = {
  ...
  content: {
    cards: [
      { cardId: 'sc-001', title: 'Azure OpenAI Now in Switzerland North', whatChanged: 'Azure OpenAI expanded to Switzerland North region.', whatChangedBullets: ['Azure OpenAI deployed to Switzerland North datacenter.', 'In-country data processing now available for Swiss workloads.'], whyItMatters: 'Removes data residency objection for Swiss-regulated workloads like Schindler.', whoCares: ['Compliance', 'Security', 'IT Architecture'], nextMove: { talkTrack: 'Data residency was your blocker — that\'s now resolved with Switzerland North.', proofToAsk: 'Data residency requirements document' }, expiresAt: expiry(12), tags: ['Vendor', 'Azure', 'Data Residency'], suggestedAction: 'Lead with this to unblock compliance team.', confidence: 'High', sources: [{ label: 'Azure Updates Blog', sourceType: 'url', url: 'https://azure.microsoft.com/updates' }], simulated: true, ctas: [{ type: 'listen_microcast', microcastType: 'account', label: 'Listen (4 min)' }] },
      { cardId: 'sc-002', title: 'EU Machinery Regulation: Digital Twins by 2027', whatChanged: 'EU regulation requires machine-readable documentation for machinery.', whatChangedBullets: ['EU mandates machine-readable documentation for all machinery sold in EU.', 'Compliance deadline set for 2027 — affects elevator OEMs directly.'], whyItMatters: 'Elevator OEMs like Schindler must invest in digital twins — compliance deadline creates urgency.', whoCares: ['Compliance', 'Engineering', 'Operations', 'Legal'], nextMove: { talkTrack: 'The 2027 deadline means your digital twin roadmap needs to start now — where are you today?', proofToAsk: 'Current compliance roadmap or gap assessment' }, expiresAt: expiry(14), tags: ['Regulatory', 'EU', 'Compliance'], suggestedAction: 'Position AI readiness assessment as compliance prerequisite.', confidence: 'High', sources: [{ label: 'European Commission', sourceType: 'url', url: 'https://ec.europa.eu' }], simulated: true, ctas: [{ type: 'listen_microcast', microcastType: 'account', label: 'Listen (4 min)' }, { type: 'listen_microcast', microcastType: 'industry', label: 'Industry Update (4 min)' }] },
      { cardId: 'sc-003', title: 'Swiss Manufacturing AI Adoption at 35%', whatChanged: 'Industry benchmark shows 35% AI adoption in Swiss manufacturing.', whatChangedBullets: ['Swiss Digital Economy Report: 35% of manufacturers now use AI in production.', 'Up from 22% in 2025 — fastest growth in DACH region.'], whyItMatters: 'Schindler risks falling behind peers — competitive pressure angle.', whoCares: ['CTO', 'Strategy', 'Operations'], nextMove: { talkTrack: 'Your peers are at 35% AI adoption — where does Schindler sit on that curve?', proofToAsk: 'Internal AI maturity assessment or roadmap' }, expiresAt: expiry(10), tags: ['LocalMarket', 'Benchmark', 'Industry'], suggestedAction: 'Reference benchmark in executive conversations.', confidence: 'Medium', sources: [{ label: 'Swiss Digital Economy Report 2026', sourceType: 'url' }], simulated: true, ctas: [{ type: 'listen_microcast', microcastType: 'account', label: 'Listen (4 min)' }, { type: 'listen_microcast', microcastType: 'industry', label: 'Industry Update (4 min)' }] },
      { cardId: 'sc-004', title: 'Copilot for Field Service in Public Preview', whatChanged: 'Microsoft launched Copilot for Field Service in public preview.', whatChangedBullets: ['Copilot for Field Service now in public preview on Dynamics 365.', 'Supports AI-assisted dispatching and knowledge retrieval for technicians.'], whyItMatters: 'Directly relevant to Schindler\'s 20K+ field technicians.', whoCares: ['Field Service', 'IT', 'Operations'], nextMove: { talkTrack: 'Copilot for Field Service just hit preview — your 20K technicians could benefit from a scoped pilot.', proofToAsk: 'Current field service tool stack and dispatching workflow' }, expiresAt: expiry(12), tags: ['Vendor', 'Copilot', 'Field Service'], suggestedAction: 'Propose Copilot Sprint scoped to field service.', packId: 'pkg-copilot', confidence: 'Medium', sources: [{ label: 'Dynamics 365 Blog', sourceType: 'url' }], simulated: true, ctas: [{ type: 'listen_microcast', microcastType: 'account', label: 'Listen (4 min)' }] },
      { cardId: 'sc-005', title: 'Kone Partners with Google Cloud for AI', whatChanged: 'Competitor Kone announced AI partnership with Google Cloud.', whatChangedBullets: ['Kone signed strategic AI partnership with Google Cloud.', 'Focuses on predictive maintenance and building analytics.'], whyItMatters: 'Creates competitive pressure — Schindler may lose first-mover advantage on Azure.', whoCares: ['CTO', 'Strategy', 'Procurement'], nextMove: { talkTrack: 'Kone just locked in Google Cloud — how does that change your AI platform calculus?', proofToAsk: 'Competitive analysis or vendor evaluation criteria' }, expiresAt: expiry(10), tags: ['Competitive', 'Google Cloud', 'Industry'], suggestedAction: 'Use competitive urgency in next CTO conversation.', confidence: 'Medium', sources: [{ label: 'Industry news', sourceType: 'internal_note' }], simulated: true, ctas: [{ type: 'listen_microcast', microcastType: 'account', label: 'Listen (4 min)' }, { type: 'listen_microcast', microcastType: 'industry', label: 'Industry Update (4 min)' }] },
      { cardId: 'sc-006', title: 'Partner Incentive Tiers Updated for AI', whatChanged: 'Microsoft updated partner incentive bands for AI workload certifications.', whatChangedBullets: ['New rebate tiers favor partners with certified AI workloads.', '15% rebate on Azure AI consumption above $50K/month.'], whyItMatters: 'AlpNova can unlock higher rebate bands by accelerating AI deployments.', whoCares: ['Practice Lead', 'Finance', 'Sales'], nextMove: { talkTrack: 'New incentive tiers just dropped — we should review eligibility before quarter-end.', proofToAsk: 'Current Azure AI consumption and certification status' }, expiresAt: expiry(21), tags: ['Vendor', 'Incentives'], suggestedAction: 'Review updated tier requirements with practice lead.', confidence: 'High', sources: [{ label: 'Partner Center', sourceType: 'url', url: 'https://partner.microsoft.com' }], simulated: true, ctas: [{ type: 'listen_microcast', microcastType: 'account', label: 'Listen (4 min)' }] },
    ],
    simulated: true,
  },
};
